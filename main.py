...
def updateTitle():
    ...
    while True:
        try:
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(f'[Generator] - Unlocked: {unlocked} | Locked: {locked} | Unlock Rate: {round(unlocked/total*100,2)}% | Speed: {round(unlocked / ((time.time() - genStartedAs) / 60))}/min - {round(unlocked / ((time.time() - genStartedAs) / 60)*60)}/hour | Time Elapsed: {result}')
        except Exception:
            pass
        time.sleep(1)
...
if __name__ == "__main__":
    ...
    print(Colorate.Diagonal(Colors.red_to_purple, Center.XCenter("GEN") + "\n" + Center.XCenter("THIS IS A LEAKED TOKEN GENERATOR ")))
